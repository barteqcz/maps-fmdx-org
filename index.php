...
<body>
    ...
    <script>
        // Global configuration injected from PHP
    ...
    <nav class="main-nav glass">
        <div class="logo-container">
            <img src="/img_assets/logo.png" class="nav-logo" alt="FMDX Logo">
        </div>
        <div class="nav-actions">
            <?php if($is_logged_in): ?>
                <?php if($is_staff): ?>
                    <button onclick="prepareAddModal()" class="btn btn-primary">
                        <i data-lucide="plus-circle" class="nav-icon"></i>
                    </button>
                <?php endif; ?>

                <?php if($user_role === 'admin'): ?>
                    <button onclick="toggleModal('adminModal')" class="btn btn-primary">
                        <i data-lucide="user-round-cog"></i>  
                    </button>
                <?php endif; ?>
                
                <a href="/php/logout.php" class="nav-link" style="display: flex; align-items: center; gap: 8px;">
                    <i data-lucide="log-out" class="nav-icon"></i>
                </a>
            <?php else: ?>
                <button onclick="toggleModal('loginModal')" class="btn btn-outline">
                    <i data-lucide="key-round" class="nav-icon"></i>  
                    <span class="nav-text">LOGIN</span>
                </button>
                <button onclick="toggleModal('registerModal')" class="btn btn-primary">
    ...
    <button id="cancel-picking-btn" class="btn-cancel-picking" onclick="cancelPicking()">
        <i data-lucide="x" style="width: 18px; height: 18px;"></i> Cancel picking
    </button>

    <div class="filter-panel glass">
        <h3 class="panel-title is-collapsed" id="filter-header" onclick="toggleFilters()" style="cursor: pointer; user-select: none;">
            Filters
            <i id="filter-toggle-icon" data-lucide="chevron-up" class="filter-toggle-icon"></i>
        </h3>
        
        <div class="filter-group is-collapsed" id="filter-content">
            ...
            <div class="grid-2">
                <input type="number" id="station-frequency" placeholder="Frequency" class="filter-input">
                <input type="number" id="station-power" placeholder="min kW" class="filter-input">
            </div>
            
            <input type="text" id="station-pi-code" placeholder="PI code" class="filter-input">
            
            <select id="country" class="filter-input">
                <option value="" disabled selected>Detecting the country via IP...</option>
        ...
        <div id="transmitter-modal" class="modal">
            <div class="modal-content lg">
                ...
                <form method="POST" id="transmitter-form" class="form">
                    ...
                    <select name="country" id="transmitter-modal-country" class="form-input text-xs" required>
                        <option value="" disabled selected>Auto-detect from coordinates (correct manually if needed)</option>
                        <?php foreach($countries as $code => $name): ?>
                            <option value="<?php echo $code; ?>"><?php echo $code . " - " . $name; ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="admin-list">
                    <?php foreach($all_users as $u): ?>
                        <div class="admin-user-row" data-search="<?php echo htmlspecialchars(strtolower($u['username'] . ' ' . $u['id']), ENT_QUOTES, 'UTF-8'); ?>">
                            <div>
                                <div class="admin-user-name"><?php echo htmlspecialchars($u['username']); ?></div>
                                <div class="admin-user-id">UID: <?php echo $u['id']; ?></div>
                            </div>
                            <form method="POST" style="display: flex; gap: 0.5rem;">
                                <input type="hidden" name="action" value="update_user_role">
                                <input type="hidden" name="target_user_id" value="<?php echo $u['id']; ?>">
                                <select name="new_role" onchange="this.form.submit()" class="admin-select">
